import 'package:skillfloor_lms/models/all_category.dart';
import 'package:skillfloor_lms/models/sub_category.dart';
...
class Categories with ChangeNotifier {
  ...
  Future<void> fetchCategories() async {
    ...
    var url = '$BASE_URL/api/course_categories';
    try {
      ...
      for (var catData in extractedData) {
        loadedCategories.add(Category(
          ...
          title: catData['title'], //change to title from name
          thumbnail: catData['thumbnail'],
          numberOfCourses: catData['number_of_courses'], //this is sub category number
          numberOfSubCategories: catData['number_of_sub_categories'], //number_of_sub_categories
        ));
  ...
  Future<void> fetchSubCategories(int catId) async {
    ...
    var url = '$BASE_URL/api/course_categories?course_id=$catId';
    print(url);
    try {
      final response = await http.get(Uri.parse(url));
      final extractedData = json.decode(response.body) as List;
      ...
      for (var catData in extractedData) {
        loadedCategories.add(SubCategory(
          id: int.parse(catData['id']),
          title: catData['title'],
          parent: int.parse(catData['parent_course_id']),
